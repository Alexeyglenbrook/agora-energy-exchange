...
Комплект аналітичних артефактів для процесу акредитації учасників торгової платформи «Агора»: ролі, права доступу, статуси заявки та переходи між ними, а також базові бізнес-правила виконання заявок (типи ордерів, ліміти, Block Trade) для вже акредитованого трейдера.
...
- [8. Бізнес-правила](#8-бізнес-правила)
- [9. Діаграма станів заявки (UML State Diagram)](#9-діаграма-станів-заявки-uml-state-diagram)
...
† AML-відхилення — окреме, самостійне рішення фінмоніторингу на етапі перевірки документів (санкційні списки тощо), відмінне від рішення «акредитувати / відхилити»: розгляд комітету в цьому випадку минається (розділ 3). На відміну від «*», тут фінмоніторинг ухвалює рішення самостійно, тому позначка — «A».
...
Then заявка знову отримує статус «Подано» і проходить автоперевірку та маршрутизацію заново (розділ 3)
```
...
Then API повертає 403 Forbidden (ACCESS_DENIED), система відхиляє дію з помилкою доступу, оскільки право «A» на цю дію (розділ 2) має лише роль «Біржовий комітет»
```
...
| 19 | Повторне подання після повернення заявки | Заявка має статус «Повернено заявнику» | Довантажити відсутній документ / усунути зауваження, повторно подати заявку | Заявка знову отримує статус «Подано» і проходить автоперевірку заново; заявник не заповнює анкету з нуля | US-01 (сценарій 5), розділ 3 | ☐ |
...
| `504 Gateway Timeout` | Формування договорів не завершилось вчасно, потрібне ручне втручання | `{"code": "OPERATION_TIMEOUT", "message": "Операція не завершилась вчасно, створено завдання для ручного втручання"}` | US-04, сценарій 4 |
| `504 Gateway Timeout` | Активація доступу до БЕТС не завершилась вчасно після трьох автоматичних спроб | `{"code": "ACTIVATION_TIMEOUT", "message": "Активація не вдалась після трьох спроб, створено завдання для ручного втручання"}` | US-06, сценарій 3 |
...
Наскрізні бізнес-правила процесу акредитації та торгів, зведені окремо від user stories, щоб їх можна було перевіряти і трасувати незалежно від конкретного сценарію. ID використовуються для трасування з UAT-чеклістом (розділ 6) та прототипом.
...
| **BR-02** | **Приватні дані учасника (контакти, ліміти, заявки на акредитацію) видимі лише персоналу біржі та самому учаснику щодо власних даних; дії «Акредитувати» / «Призупинити» / «Відновити» доступні лише персоналу** | Кабінет учасника, картка учасника, реєстр (розділ 2, US-08 сценарій 2) | Роль «Трейдер» не бачить приватних даних інших учасників і не може викликати керуючі дії над чужою акредитацією — головне правило видимості, яке демонструє цей прототип |
| BR-03 | Заявка з підтвердженою наявністю заявника в санкційному списку відхиляється автоматично, минаючи розгляд Біржового комітету | AML-перевірка (US-03, сценарій 2; розділ 3) | Не допускає колегіального розгляду заявки з підтвердженою AML-підставою для відмови |
...
| BR-05 | Обсяг заявки на купівлю/продаж не може перевищувати вільний ліміт учасника (загальний ліміт мінус використаний, спільний по всіх торгових платформах) | Виставлення заявки в «Торги» (US-07, сценарій 2) | Блокує подання заявки з перевищенням вільного ліміту |
| BR-06 | Ціна лімітної або стоп-заявки має бути додатним числом | Виставлення заявки (US-07, сценарій 3) | Заявка з порожньою або від'ємною ціною не подається |
| BR-07 | Ринкова заявка виконується негайно за найкращою зустрічною ціною в межах доступного обсягу (IOC); невиконаний залишок скасовується і не потрапляє в стакан | Тип заявки «Ринкова» | Часткове виконання допускається, часткове очікування в стакані — ні |
| BR-08 | Стоп-заявка перетворюється на ринкову лише в момент, коли ціна останньої угоди перетинає вказану стоп-ціну | Тип заявки «Стоп» | До спрацювання тригера заявка не видима і не бере участі в торгах |
| BR-09 | Айсберг-заявка показує в стакані лише вказану видиму частину обсягу; після виконання видимої частини наступна порція з прихованого залишку додається в кінець черги за ціною (втрачає пріоритет часу подання) | Тип заявки «Айсберг» | Прихований обсяг ніколи не відображається в стакані одночасно з видимою частиною |
| BR-10 | Block Trade реєструється лише за обсягу не менше встановленого мінімуму (200 одиниць обсягу ринку) і не може перевищувати вільний ліміт жодної зі сторін угоди; резервує ліміт обох сторін і не потрапляє до публічного стакану | Режим «Block Trade» | Мінімальний поріг обсягу та ліміт обох контрагентів; комісія клірингу 0.05% від суми угоди |

**BR-02** навмисно виділено — це те саме єдине жорстке бізнес-правило, навколо якого спроєктований прототип (розділ 2, матриця доступів): видимість чужих приватних даних і керуючі дії над акредитацією обмежені виключно роллю персоналу.

## 9. Діаграма станів заявки (UML State Diagram)

Діаграма станів для заявки на акредитацію / ордера — той самий перелік станів і переходів, що й у таблиці розділу 3, у вигляді UML state diagram.
